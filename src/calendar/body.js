...
const TimeCell = ({ slot, index }) => {
  ...
  return (
    <div
      className={`d-flex justify-content-end align-items-center ${classes.timeCellText}`}
    >
      <div className='d-flex justify-content-end align-items-center'>
        <Typography
          variant='caption'
          className='font-weight-500 fmr-10 text-uppercase'
          color='textSecondary'
        >
          {slot}
        </Typography>
    ...
    <TableBody className={classes.bodyContainer}>
      {slots.map((slot, index) => (
        <TableRow key={slot + index} className={classes.row}>
          {
            <TableCell
              className={`${classes.timeSlotColumn} ${
                hideGrid || index === 0 ? classes.hideBorderTop : ''
              } `}
            >
