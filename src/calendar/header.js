...
const CalendarHeader = (props) => {
  ...
  const classes = useCalendarStyles();
  const cellClasses = (date) => clsx((moment(date.date).isSame(moment(), 'day') && classes.todayCell), classes.columnCell, 'position-sticky')
  return (
    <TableHead className={classes.tableHead}>
      <TableRow className={classes.headerRow}>
        ...
        {dates.map((date) => (
          <TableCell
            key={date.id}
            className={cellClasses(date)}
          >
            {Boolean(HeaderCellComponent) ? (
