...
export const RightScrollButton = props => {
  ...
  function updateRightScrollButton(){
    ...
    if(scrollWidth > offsetWidth && (Math.round(scrollLeft) === (scrollWidth - offsetWidth))) return setRightScroll(false);
  ...
  return (
    <Fade in={showRightScrollButton}>
      <div className={classes.rightScrollerContainer + ' border-bottom position-absolute'} onClick={scrollRight}>
...
export const LeftScrollButton = ({...restProps}) => {
  ...
  const classes = useCalendarStyles();
  return (
    <div className={classes.leftScrollerContainer + ' d-flex align-items-center justify-content-center border-bottom position-absolute'} onClick={scrollLeft}>
      <IconButton size='small'>
        <KeyboardArrowLeft />
      </IconButton>
    </div>
