...
const useSessionStyles = makeStyles((theme) => ({
  sessionContainer:{
    position: 'absolute',
    top: ({ top, isConflicted }) => isConflicted ? `${((top - (CELL_HEIGHT / 2)) + 5)}px`
      : `${top - (CELL_HEIGHT / 2)}px`, // subtract CELL_HEIGHT / 2 as the time mark is top border of the table cell
    left:({ left, isConflicted }) => isConflicted ? `${left + 30}px` : `${left}px`,
    width: ({isConflicted}) => isConflicted ? `${CELL_WIDTH - 30}px` :`${CELL_WIDTH}px`,
...
const Session = ({session, dates, timeInterval, sessionComponent: SessionComponent, parentSession, isConflicted}) => {
  ...
  const classes = useSessionStyles({ left: coordinates.x, top: coordinates.y, height, isConflicted });

  if(!height) return null;
  return (
    <div className={classes.sessionContainer}>
      {Boolean(SessionComponent) ? (
        <SessionComponent session={parentSession ? parentSession : session} isConflicted={isConflicted}/> 
      ) : ( 
        <div className={classes.defaultSession}>
          <Typography variant='caption'>
            {moment(session.startDate).format('h:mm a')} to {moment(session.endDate).format('h:mm a')}  
          </Typography>
        </div>  
      )}
    </div>
...
export const Sessions = prop => {
  ...
  const renderEvents = (session) => {
    const isEncountered = alreadyEncounteredTime[session.startDate.toISOString()]; // check if any session at this time was already encountered
    ...
    if(sessionStart.isBefore(firstDay ,'day') || sessionStart.isAfter(lastDay ,'day'))
      return null;
    ...
    if(sessionEnd.isAfter(sessionStart, 'day')){
      ...
      let newSessions = [
        { ...session, startDate: sessionStart.clone().toDate(), endDate: sessionStart.clone().endOf('day').toDate()},
        { ...session, startDate: sessionEnd.clone().startOf('day').toDate(), endDate: sessionEnd.clone().toDate()}
      ];
